# Terms of Use

## Copyright

All code, files, documents, and other materials contained in this repository (“the Code”) are the copyright of EfaYukacoonar (the owner of this account).

## Non‑Commercial Use

You are permitted to copy, modify, and use the Code **for non‑commercial purposes only**.
“Non‑commercial” refers to any use that does **not** generate revenue, profit, or compensation, whether directly or indirectly.

## Prohibition of Commercial Use

Commercial use of the Code is strictly prohibited.
“Commercial use” includes any activity that results in revenue generation, such as:

1. Software, services, or products that generate income and incorporate the Code
2. Any derivative work that leads to advertising revenue, donations, sales, or paid features
3. Any revenue generated as a result of using, referencing, or modifying the Code
(regardless of intent)


If you wish to use the Code for commercial purposes, you must contact EfaYukacoonar in advance and obtain written permission.

## Contributions

Copyright for contributions (such as pull requests) remains with the contributor.
However, contributors are deemed to grant EfaYukacoonar a non‑exclusive, royalty‑free license to use, modify, and redistribute their contributions within this repository.

## Disclaimer

The Code is provided “as is,” without any warranties of any kind.
EfaYukacoonar assumes no responsibility for any damages arising from the use of the Code.